...
public class Classifier {
    ...
    //模型中输入变量的名称
    private static final String inputName = "conv1/biases";
    //模型中输出变量的名称
    private static final String outputName = "softmax_linear/softmax_linear";
    ...
    public float[] getPredict(Bitmap bitmap) {
        ...
        inferenceInterface.feed(inputName, inputdata, IN_COL, IN_ROW,3);
        //运行tensorflow
        String[] outputNames = new String[] {outputName};
        inferenceInterface.run(outputNames,false);
        ///获取输出节点的输出信息
        float[] outputs = new float[3000]; //用于存储模型的输出
    ...
    public static float[] bitmapToFloatArray(Bitmap bitmap, int rx, int ry){
        ...
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                ...
                result[j * height* 3 + 3 * i + 0] = r / 1.0f;
                result[j * height* 3 + 3 * i + 1] = g / 1.0f;
                result[j * height* 3 + 3 * i + 2] = b / 1.0f;
            }
        }
        return result;
    }
